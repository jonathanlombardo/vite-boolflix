...
export const store = reactive({
  ...
  async fetchMovies() {
    return new Promise((resolve) => {
      ...
      const config = {
        ...
        url: `https://api.themoviedb.org/3/search/movie?query=${searchText.split(" ").join("%20")}&include_adult=${this.isAdult}&language=${this.lang}`,
        headers: {
      ...
      axios
        .request(config)
        .then((response) => {
          resolve((this.foundedMovies = response.data.results));
        })
        .catch((error) => {
        ...
        .finally(() => {
          ...
          if (searchText != this.searchText) this.fetchMovies();
          resolve();
        });
    });
  },
  ...
  async fetchSeries() {
    return new Promise((resolve) => {
      ...
      const config = {
        method: "get",
        maxBodyLength: Infinity,
        url: `https://api.themoviedb.org/3/search/tv?query=${searchText.split(" ").join("%20")}&include_adult=${this.isAdult}&language=${this.lang}`,
        headers: {
          Authorization: `Bearer ${tmdbApiToken}`,
        },
      };
      ...
      axios
        .request(config)
        .then((response) => {
          resolve((this.foundedSeries = response.data.results));
        })
        .catch((error) => {
          resolve(console.error("La chiamata axios ha riportato: ", error));
        })
  ...
  async fetchFilms() {
    ...
    this.foundedMovies.forEach((movie) => {
      ...
      this.foundedFilms.push(myMovieObj);
    });
    ...
    this.foundedSeries.forEach((serie) => {
      ...
      this.foundedFilms.push(mySerieObj);
    });
